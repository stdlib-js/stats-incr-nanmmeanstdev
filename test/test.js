...
tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrnanmmeanstdev( 3 ), 'function', 'returns expected value' );
	t.end();
});

tape( 'the function returns an accumulator function (output)', function test( t ) {
	t.strictEqual( typeof incrnanmmeanstdev( [ 0.0, 0.0 ], 3 ), 'function', 'returns expected value' );
	t.end();
});
...
tape( 'if not provided an input value, the accumulator function returns the current arithmetic mean and corrected sample standard deviation', function test( t ) {
	...
	tol = EPS * expected[ 1 ];
	t.strictEqual( delta < tol, true, 'expected: '+expected[ 1 ]+'. actual: '+actual+'. tol: '+tol+'. delta: '+delta+'.' );
...
tape( 'if only one datum has been provided, the accumulator function returns `0` for the sample standard deviation', function test( t ) {
	var acc = incrnanmmeanstdev( 3 );
	var v = acc( 2.0 );
	t.strictEqual( v[ 0 ], 2.0, 'returns expected value' );
...
tape( 'if the window size is `1`, the accumulator functions always returns `0` for the sample standard deviation', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		v = randu() * 100.0;
		out = acc( v );
		t.strictEqual( out[ 0 ], v, 'returns expected value' );
...
tape( 'if provided `NaN`, the accumulator returns current values or null', function test( t ) {
	...
	for ( i = 0; i < data.length; i++ ) {
		...
		if ( i === 0 && isnan( data[ i ] ) ) {
			...
			if ( isnan( data[ i ] ) ) {
				t.strictEqual( isnan( v[ 0 ] ), false, 'returns expected value for window '+i );
				t.strictEqual( isnan( v[ 1 ] ), false, 'returns expected value for window '+i );
			}

			t.strictEqual( v[ 0 ], expected[ i ][ 0 ], 'returns expected value for window '+i );
			t.strictEqual( v[ 1 ], expected[ i ][ 1 ], 'returns expected value for window '+i );
